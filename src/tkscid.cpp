...
sc_game_list (ClientData cd, Tcl_Interp * ti, int argc, const char ** argv)
{
    bool showProgress = startProgressBar();
    if (argc < 5  &&  argc > 7) {
        return errorResult (ti, "Usage: sc_game list <start> <count> <format> [<bool> [<file>]]");
    }
    if (! db->inUse) { return TCL_OK; }
    ...
    // Load each game and lookup next 6 moves 
    bool nextMoves = 0;
    if (argc >= 6) 
       nextMoves = strGetBoolean(argv[5]);
...
sc_pos (ClientData cd, Tcl_Interp * ti, int argc, const char ** argv)
{
    static const char * options [] = {
        ...
        "probe", "setComment", "side", "tex", "moves", "movesUci", "location", "getScore", NULL
    };
    enum {
        ...
        POS_SETCOMMENT, POS_SIDE, POS_TEX, POS_MOVES, POS_MOVES_UCI, LOCATION, POS_SCORE
    };
    ...
    case POS_SCORE:
        // duplicate get-score logic from proc addScoreToList S.A.
        tempStr = db->game->GetMoveComment();
        if (!tempStr) {
	    Tcl_AppendResult (ti, "", NULL);
        } else {
	    ...
	    if (*tempStr == 0  || ! isdigit(*(tempStr+1)) ||
		  (*tempStr == '-' && isdigit(*(tempStr-1)))) {
		Tcl_AppendResult (ti, "", NULL);
	    } else {
		// OK, now we have found "+[digit]" or "-[digit]" in the tempStr,
		float f;
		sscanf (tempStr, "%f", &f);
		sprintf (boardStr, "%.2f", f);
		Tcl_AppendResult (ti, boardStr, NULL);
	    }
        }
	break;
...
//    Analyzing a random position is now possible with "-board Board[66]", which returns absolute score S.A.
int
sc_pos_analyze (ClientData cd, Tcl_Interp * ti, int argc, const char ** argv)
{
    ...
    static const char * options [] = {
        "-time", "-hashkb", "-pawnkb", "-post", "-pruning", "-mindepth", "-searchdepth", "-board", NULL
    };
    enum {
        OPT_TIME, OPT_HASH, OPT_PAWN, OPT_POST, OPT_PRUNING, OPT_MINDEPTH, OPT_SEARCHDEPTH, OPT_BOARD
    };
    int arg = 2;
    while (arg+1 < argc) {
        const char * option = argv[arg];
        ...
        switch (index) {
            ...
            case OPT_BOARD:    strncpy(board,value,66); board[66]=0; break;
            default:
    ...
    if (*board) {
      pos = new Position;
      pos->Init();
      if (pos->ReadFromLongStr((const char*)board) != OK) {
        delete pos;
        return TCL_ERROR;
      }
    } else {
      pos = db->game->GetCurrentPos();
    }
    ...
    // -board option returns absolute score
    if (*board && board[65] == 'b')
      score = -score;
